...
.factory('pusher', function($http, config, Api, $window, $q, $rootScope, pusherCore) {
	...
	var pusher = angular.extend(pusherCore, {});
	...
	pusher.subscribeToChannel = function(channelId, channelSwitch, callback) {
		...
		if (!config.settings.NoPusherOnChannelWeb) {
			pusher.subscribe('presenceChannel', 'public-on-channel_' + pusher.channelId + '_' + pusher.shard + '_' + channelSwitch);
		}
