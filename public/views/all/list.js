...
(function () {
	 ...
	 */
	angular.module('mytodoApp').controller('TodoListCtrl', ['$state', 'TodoService', 'AuthService', function($state, TodoService, AuthService) {
		...
		self.like = function(_id) {
			console.log('_id',_id);
			TodoService.like(_id);
		};

		TodoService.getEvents().then(function(data){
			if (data.length && Array.isArray(data)) {
				self.events = data;
			}
		}, TodoService.getFail);

		TodoService.getLikes().then(function(data){
			if (data.length && Array.isArray(data)) {
				self.likes = data;
			}
		}, TodoService.getFail);
