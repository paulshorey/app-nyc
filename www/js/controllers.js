		...
		*/
		...
		EventService.getEvents(query).then(function(response){
			...
			for (var each in events) {
				...
				html += '		<a class="events-event event-link" href="'+event.link+'" target="_blank" onclick="window.open(‘'event.link'’, ‘_blank’)" prevent-default style="background-image:url('+event.image+');">\n';
